...
PERMISSIONLEVELFORGROUP_ONE_OF_SCHEMAS = ["CanManage", "CanManageRun", "CanView"]
...
class PermissionLevelForGroup(BaseModel):
    ...
    one_of_schemas: List[str] = Field(
        PERMISSIONLEVELFORGROUP_ONE_OF_SCHEMAS, const=True
    )
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into PermissionLevelForGroup with oneOf schemas: CanManage, CanManageRun, CanView. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into PermissionLevelForGroup with oneOf schemas: CanManage, CanManageRun, CanView. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PermissionLevelForGroup:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> PermissionLevelForGroup:
